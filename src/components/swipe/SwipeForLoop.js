...
export default React.createClass({
    ...
    autoplay() {
        ...
        if (Platform.OS === 'android') {
            ...
            this.setTimeout(()=> {
                ...
                var y = this.state.index * this.props.height ;
                if (index < this.state.total) {
                    index += 1;
                } else {
                    index = 0
                }
                this.refs && this.refs.scrollView && this.refs.scrollView.scrollTo({x: 0, y:y, animated: true});
                this.setState({
                    index
                });
                this.autoplay()
    ...
    renderButtons() {
        ...
        let nextButton = this.props.nextButton || <Text
                style={[styles.buttonText, {color: !this.props.loop && this.state.index == this.state.total - 1 ? 'rgba(0,0,0,0)' : '#007aff'}]}>›</Text>

        let prevButton = this.props.prevButton || <Text
                style={[styles.buttonText, {color: !this.props.loop && this.state.index == 0 ? 'rgba(0,0,0,0)' : '#007aff'}]}>‹</Text>

        return (
            <View pointerEvents='box-none'
                  style={[styles.buttonWrapper, {width: this.state.width, height: this.state.height}, this.props.buttonWrapperStyle]}>
                <TouchableOpacity
                    onPress={() => !(!this.props.loop && this.state.index == 0) && this.scrollTo.call(this, -1)}>
                    <View>
                        {prevButton}
                    </View>
                </TouchableOpacity>
                <TouchableOpacity
                    onPress={() => !(!this.props.loop && this.state.index == this.state.total - 1) && this.scrollTo.call(this, 1)}>
                    <View>
                        {nextButton}
                    </View>
                </TouchableOpacity>
